...
def start():
    ...
    root.wm_iconphoto(True, ImageTk.PhotoImage(image = Image.open(urllib.request.urlopen("https://raw.githubusercontent.com/CrazyFox7048/LVCAppStore/main/2_logo_west_suffolk_Ye0_icon.ico"))))
...
def logon():
    ...
    ttk.Label(root, text="West Suffolk College App Store").grid(row=0, column=1, columnspan = 2, padx=5, pady=5)

    ttk.Label(root, text='WSC Username:').grid(row=1, column=0, pady=5)
...
def DownloadStart(link, name, destination, shortcut, run, message, button, msg, msg1):
    ...
    if shortcut != "":
        shortcutTarget = file[ : -4]
        shortcutTarget = "C:\\Users\\w23723111\\Documents\\" + shortcutTarget + ".lnk"
        if not os.path.exists(shortcutTarget):
            createShortcut(shortcutTarget, shortcut)
            Label(root, text="Shortcut created in your home area", bg=bg, fg=fg).grid(row=3, column=0, columnspan=3, padx=5, pady=5)
        else:
            Label(root, text="Shortcut already exists", bg=bg, fg=fg).grid(row=3, column=0, columnspan=3, padx=5, pady=5)
    else:
        Label(root, text="No shortcut created", bg=bg, fg=fg).grid(row=3, column=0, columnspan=3, padx=5, pady=5)
    Label(root, text=message, bg=bg, fg=fg).grid(row=4, column=0, padx=5, pady=5)
    if run:
        subprocess.Popen([admin, destination + file])

def createShortcut(shortcut_path, target, arguments='', working_dir=''):
...
def manualDownload():
    ...
    Label(root, text='Shortcut Path:', bg=bg, fg=fg).grid(row=3, column=0, padx=5, pady=5)
    shortcut.grid(row=3, column=1, padx=5, pady=5)
    ttk.Checkbutton(root, text='Run file after instalation?', takefocus=0, variable = run).grid(row=4, column=0, padx=5, pady=5)
    ttk.Button(root, image=enterIcon, command=lambda: manualDownloadStart(link, destination, shortcut, run)).grid(row=5, column=1, padx=5, pady=5)
    ttk.Button(root, image=backIcon, command=lambda: page2(user)).grid(row=5, column=0, padx=5, pady=5)
...
def manualDownloadStart(link, destination, run):
    ...
    if link != "" and destination != "":
        ...
        Label(root, text='Link and Path feilds must be filled!', bg=bg, fg="red").grid(row=5, column=0, padx=5, pady=5)
        return
